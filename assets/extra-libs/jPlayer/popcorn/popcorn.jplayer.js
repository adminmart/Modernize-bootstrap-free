...
(function (Popcorn) {
  var JQUERY_SCRIPT = "//code.jquery.com/jquery-1.11.0.min.js", // Used if jQuery not already present.
    ...
    format = {
      // Duplicate of jPlayer 2.5.0 object, to avoid always requiring jQuery and jPlayer to be loaded before performing the _canPlayType() test.
      mp3: {
    ...
    getSupplied = function (url) {
      // Function to generate a supplied option from an src object. ie., When supplied not specified.
      var supplied = "",
        separator = "";
      if (isObject(url)) {
          ...
          for (var sol = 0; sol < solution.length; sol++) {
            ...
            for (var fmt = 0; fmt < supplied.length; fmt++) {
              ...
              if (format[mediaType]) {
                ...
                var htmlCanPlay = !!(
                    mediaElem &&
                    mediaElem.canPlayType &&
                    mediaElem.canPlayType(format[mediaType].codec)
                  ),
                  htmlWillPlay = htmlCanPlay && checkingHtml,
                  flashWillPlay =
                    format[mediaType].flashCanPlay && checkingFlash;
      ...
      var media = this,
        ...
        dispatchDurationChange = function () {
          if (ready) {
            ...
            if (DEBUG)
              console.log(
                "DELAYED EVENT (!ready) : durationchange : " + duration
              );
        ...
        jPlayerFlashEventsPatch = function () {
          ...
          myPlayer
            .bind($.jPlayer.event.loadstart, function () {
              setTimeout(function () {
        ...
        jPlayerInit = function () {
          (function ($) {
            ...
            if (typeof media.src === "string") {
              ...
              jpOptions.supplied =
                jpOptions.supplied || getSupplied(media.src.media);
            }
                ...
                if (DEBUG)
                  console.log(
                    "Dispatched event: " +
                      name +
                      (event && event.jPlayer
                        ? " (" + event.jPlayer.status.currentTime + "s)"
                        : "")
                  ); // Must be after dispatch for some reason on Firefox/Opera
            ...
            for (var eventName in $.jPlayer.event) {
              if ($.jPlayer.event.hasOwnProperty(eventName)) {
                ...
                if (nativeEvent) {
                  ...
                  if (DEBUG)
                    console.log(
                      "Skipped auto event handler creation for: " + eventName
                    );
                }
              }
            }
